...
def prepare_from_imported(
    parquet_organisations: str,
    parquet_activites: str,
    parquet_exercices: str,
    parquet_domaines: str,
    parquet_decisions: str,
) -> tuple[pd.DataFrame, pd.DataFrame]:
    # --- Load
    df_organisations = pd.read_parquet(parquet_organisations).set_index("representants_id")
    df_activites = pd.read_parquet(parquet_activites).set_index("activite_id")
    df_exercices = pd.read_parquet(parquet_exercices).set_index("exercices_id")
    df_domaines_intervention = pd.read_parquet(parquet_domaines).set_index("activite_id")
    df_decisions = pd.read_parquet(parquet_decisions)
    ...
    # =========================
    # 3) DECISIONS PUBLIQUES
    # =========================
    for col in ["Date initiale", "Date de promulgation"]:
        if col in df_decisions.columns:
            df_decisions[col] = pd.to_datetime(df_decisions[col], errors="coerce", dayfirst=True)
    if "type_decision" not in df_decisions.columns:
        df_decisions["type_decision"] = "loi"
    allowed = {"loi", "decret", "ordonnance", "arrete"}
    df_decisions["type_decision"] = df_decisions["type_decision"].astype("string").str.lower().str.strip()
    df_decisions = df_decisions[df_decisions["type_decision"].isin(allowed)].copy()

    return df_activites, df_decisions
...
def minify_lois(df_lois: pd.DataFrame) -> pd.DataFrame:
    keep = ["decision_id", "type_decision", "Date initiale", "Date de promulgation", "Titre", "Description", "Numéro de la loi", "Thèmes", "État du dossier", "URL du dossier", "nor", "source"]
    cols = [c for c in keep if c in df_lois.columns]
    out = df_lois.loc[:, cols].copy()
    for c in ["decision_id", "type_decision", "Titre", "Description", "Thèmes", "État du dossier", "URL du dossier", "Numéro de la loi", "nor", "source"]:
        if c in out.columns:
...
def build_docs_lois(df_lois_min: pd.DataFrame) -> pd.DataFrame:
    # La description n'est pas garantie sur tous les textes Tricoteuses.
    # On garde l'axe Titre + Description demandé, avec Thèmes en secours
    # pour éviter une régression forte de rappel BM25 quand la notice manque.
    doc_text = (
        df_lois_min.get("Titre", "").fillna("").astype(str)
        + " "
        + df_lois_min.get("Description", "").fillna("").astype(str)
        + " "
