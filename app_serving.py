...
with st.expander("ℹ️ À propos de l'application", expanded=False):
        ...
        Elle est développée par Vincent Castaignet, data analyst/scientist freelance.
        """)
...
with st.expander("ℹ️ Comment ça marche ?", expanded=False):
        ...
        - lire les résultats (objet_activite), et désélectionner les activités qui ne sont pas pertinentes (colonne "Sélection"), puis cliquer sur "Valider la sélection" pour confirmer les activités retenues
        - faire de même pour les lois correspondant à la recherche (désélectionner les lois non pertinentes, puis valider)
        ...
        - utiliser les filtres pour ajuster la recherche: nombre de résultats tTop N, budget, période, score lexical et sémantique, poids du score sémantique.
        La 3ème ligne de paramètres concerne des paramètres techniques de la recherche vectorielle (FAISS nprobe, mode de fusion des scores BM25 et vectoriels, etc.) qui peuvent être laissés à leur valeur par défaut pour une utilisation classique. 
        """)
...
with st.expander("Version artefacts", expanded=False):
...
with tab_search:
    ...
    c4, c5 = st.columns(2)
    with c4:
        min_bm25 = st.slider("Score lexical min (BM25)", 0.0, 20.0, 8.0, 0.1)
    with c5:
        min_vec  = st.slider("Score sémantique min (vec)", -1.0, 1.0, 0.5, 0.01)

    c6, c7, c8, c9 = st.columns(4)
    with c6:   
        nprobe = st.slider("FAISS nprobe", 1, 64, 16, 1)
    with c7:
        alpha_vec = st.slider("Poids sémantique", 0.0, 1.0, 0.55, 0.05)
    with c8:
        fusion_mode = st.selectbox("Fusion", ["rrf", "mean"], index=0)
    with c9:
        rrf_k = st.slider("RRF k", 10, 200, 60, 5) if fusion_mode == "rrf" else 60
    ...
    with st.expander("Paramètres Lois", expanded=False):
        ...
        k_bm25_laws = st.slider("Candidats BM25 (lois)", 50, 1000, 200, 50)
        k_vec_laws = st.slider("Candidats vecteur (lois)", 50, 1000, 200, 50)

        min_bm25_laws = st.slider("Seuil BM25 (lois)", 0.0, 50.0, 8.0, 1.0)
        min_vec_laws = st.slider("Seuil vec_score (lois)", -1.0, 1.0, 0.5, 0.05)  # cosine approx si normalize
        min_hybrid_laws = st.slider("Seuil hybrid_score (lois)", 0.0, 1.0, 0.0, 0.05)
